...
def apply_daily_decay_if_needed():  
    ...
    with FileLock(STATE_LOCK_FILE):  
        ...
        # -----------------------------  
        if not state.get("initialized"):                                           state["remaining"] = max_allowed  
        ...
        if int(state.get("remaining", 0)) <= 0:  
            state["remaining"] = 0                                                 state["last_calc_date"] = today  
            save_ticket_state(state)  
            return state  
...
EVENT_DATE = "Dec 30, 2025"                                            EVENT_PLACE = "District of Colombia, DC, United States"  
EVENT_TIME = "5PM"  
...
# HELPERS  
# --------------------------------------------------  
ORDERS_INDEX_FILE = os.path.join(TICKET_STORAGE_DIR, "orders.json")    ORDERS_INDEX_KEY = "indexes/orders.json"  
...
def save_orders_index(data):  
    ...
    if r2_client:  
        try:  
            r2_client.put_object(  
                Bucket=R2_BUCKET_NAME,  
                Key=ORDERS_INDEX_KEY,                                                  Body=payload,  
                ContentType="application/json",  
            )  
            return  
        except Exception as e:  
...
def load_used_orders():  
    ...
    if r2_client:  
        try:  
            ...
            return set(json.loads(obj["Body"].read()))                         except Exception:  
            pass  
...
def register_order(order_id, email, files, product, quantity, ticket_numbers):  
    ...
    if order_id not in email_index[email]:                                     email_index[email].append(order_id)  
...
def load_used_nonces():  
    if r2_client:                                                              try:  
            ...
            return json.loads(obj["Body"].read())                              except Exception:  
            pass  
    return {}  
...
def verify_paypal_order(order_id, expected_amount):  
    ...
    if currency != "USD":                                                      return False, f"Invalid currency ({currency})"  
...
def generate_ticket_with_placeholders(                                     full_name, ticket_no, event_date, ticket_price, event_place, event_time  
):  
...
def send_ticket_file(order_id, enforce_limit=False):  
    ...
    if not files:                                                              return jsonify({"error": "Ticket not found"}), 404  
...
def verify_request_nonce(req):                                             """  
    ...
    try:  
        ...
        if abs(now - ts) > NONCE_EXPIRY_SECONDS:                                   return False, "Request expired"  
    except:  
    ...
    # Store nonce                                                          USED_NONCES[nonce] = ts  
...
def ticket_state():  
    ...
    payload = {  
        ...
        "last_calc_date": state.get("last_calc_date"),                         "initialized": state.get("initialized", False),  
        "today": today,  
        "cache": "MISS"  
    }  
...
@limiter.limit("5 per 10 seconds")                                     def tickets_sold():  
...
@limiter.limit("5 per minute")                                         def record_sale():  
...
def health_check():  
    return jsonify({  
        "status": "ok",  
        "service": "raffle-api",                                               "version": "1.0.0",  
        "time": datetime.utcnow().isoformat() + "Z"  
    }), 200  
  
  
def order_already_generated(order_id):                                     order_dir = os.path.join(TICKET_STORAGE_DIR, order_id)  
...
def generate_ticket():  
    ...
    try:  
        if quantity == 1:  
            ...
            pdf = generate_ticket_with_placeholders(                                   full_name,  
                ticket_no,  
                EVENT_DATE,  
                str(ticket_price),  
                event_place,  
        ...
        zip_path = os.path.join(order_dir, f"RaffleTickets_{order_id}.zip")  
        with open(zip_path, "wb") as f:                                            f.write(zip_stream.getvalue())  
        ...
        register_order(  
            ...
            product=product_title,  
            quantity=quantity,                                                     ticket_numbers=ticket_numbers,  
        )  
...
@app.route("/download_ticket", methods=["POST"])                       @limiter.limit("10 per minute")  
def download_ticket():  
    ok, err = verify_request_nonce(request)  
    if not ok:  
        return jsonify({"error": err}), 403  
...
def my_tickets():  
    ok, err = verify_request_nonce(request)                                if not ok:  
        return jsonify({"error": err}), 403  
